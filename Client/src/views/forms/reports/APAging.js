    ...
    const fetchAPAging = async () => {
        try {
            ...
            const formatted = bills.map(bill => {
                const daysOverdue = bill.dueDate ? Math.floor((new Date() - new Date(bill.dueDate)) / (1000 * 60 * 60 * 24)) : 0
                const bucket =
                ...
                return {
                    vendor: bill.vendor?.displayName || 'Unknown',
            ...
            setError('Failed to load A/P Aging report')
            setLoading(false)
        }
    }
    ...
    return (
        <>
            <CCard className="mb-4">
                <CCardHeader>
                    <h4>Accounts Payable Aging Report</h4>
                            ...
                            {aging.length === 0 ? (
                                ...
                                aging.map((row, idx) => (
                                    <CTableRow key={idx}>
                                        ...
                                        <CTableDataCell>{row.billNumber}</CTableDataCell>
                                        <CTableDataCell>{new Date(row.billDate).toLocaleDateString()}</CTableDataCell>
                                        <CTableDataCell>{row.dueDate ? new Date(row.dueDate).toLocaleDateString() : '-'}</CTableDataCell>
                                        <CTableDataCell>
                                            {row.daysOverdue > 0 ? (
